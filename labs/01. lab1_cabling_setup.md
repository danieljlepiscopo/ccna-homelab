...
- As you can see, on Vlan99, our current setup:
    1. IPv6 `FD00:1:0:1::1/64` has been added with `ipv6 enable` command.
    2. OSPFv3 has been added with the command `ipv6 ospf 1 area 0`.
...
**So, OSPFv3 sees the SVI as inactive for OSPFv3 purposes.**
...
I’ll probably try to sell the classic 3560 or give it away to someone else studying for the CCNA.
...
Next, we’re going to configure the 3560v2 with the 3560C configuration. 
...
After that, everything was up and running! Lastly, we had to make sure we ran all of the show commands and verify that the output is the same on the new 3560v2.
...
## New POE Access Switch

As I was getting ready to put together my new Cisco 2504 Wireless Controller and the Cisco 3802i Access Point for my Wireless LAN (WLAN) lab, I connected everything correctly:

`SW1 Fa0/4 ←→ WLC Port 1`

`SW1 Fa0/6 ←→ AP`

<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/d30254a0-08ee-4403-b2eb-30103f663dd3" />
</center>

However, my SW1 (Cisco Catalyst 3560 v2) is the only switch in my lab that has POE, which is my Core layer. Most APs will connect at the Access layer, so to be more uniform with an enterprise network, it would be wise to do another migration and have a new POE as my SW2. 

So, that’s what we’re going to do!

<center>
  <img width="400" height="500" alt="new_3560v2_switch" class="center" src="https://github.com/user-attachments/assets/9f894179-91cd-4732-a598-8bb3b441f1f8" />
</center>

- My coworker gave me a couple of his networking devices for my birthday this year, one of them being this Cisco Catalyst 3560 Series POE-24 (WS-C3560-24PS-S)

We’ll do a similar migration to the one before, but since this will technically be an L2 switch, we won’t have routing on this device. Thus, making this migration a lot easier than the last.

### The Migration

Now that we have the new device, we need to:

1. **Save Configuration from the 2960**
2. **Stage the New 3560w/POE**
3. **Add the Configuration from 2960 to 3560w/POE**
4. **Physically Swap Devices**
5. **Verify**

Now that we have our steps, let’s take a look at our current 2960.

**Save Configuration from the 2960**

First, let's save all of the running configuration to a text file, just in case we run into any issues along the way with the new 3560. Here are the commands we’ll run:
...
- These commands will get the general running configuration as well as trunks, STP, VLANs, and our ACL.

Once we have all of this saved, we can move on to staging our new 3560!

**Stage the New 3560w/POE**

With the power cable and console cable (rollover) plugged in, let's get to Privileged EXEC mode and wipe any existing configuration with:
...
After the device is wiped, reloaded, and set to dual stack, we can move on to configuring it.

**Add the Configuration from 2960 to 3560w/POE**
With everything good to go, we can configure the new 3560w/POE from my 2960 with this massive configuration paste:

```bash
conf t
!
hostname SW2
no ip routing                 ! L2 only
ip ssh version 2
!
boot-start-marker
boot-end-marker
...
! ---- Uplinks ----
interface FastEthernet0/1
 description Uplink to SW1 (Native VLAN 99 Trunk)
 switchport trunk encapsulation dot1q
 switchport mode trunk
 switchport trunk native vlan 99
exit
...
! ---- Fiber Uplinks ----
interface GigabitEthernet0/1
 description Fiber Uplink to SW1 Gi0/1
 switchport trunk encapsulation dot1q
 switchport mode trunk
...
!
interface GigabitEthernet0/2
 description Fiber Downlink to SW3 Gi0/2
 switchport trunk encapsulation dot1q
 switchport mode trunk
 switchport trunk allowed vlan 10,20,99
exit
...
<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/4fb2f57d-5925-4c8f-a3be-0d2be3f08a1d" />
</center>
<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/c2fc6875-204e-4275-a458-67fcf5fb032a" />
</center>
...
- Swapping them was really easy, I just had to:
    1. Pull out the Ethernet and fiber connections
    2. Remove the 2960 by unscrewing the side screws
    3. Remove the rack mount ears from the 2960
    4. Add the rack mounts to the 3560w/POE
    5. Place the screws on the rack mount for the 3560w/POE
    6. Add the Ethernet and fiber connections

Ultimately, the physical topology didn’t change, only the L3 switch:

- `SW2 (3560w/POE) Fa0/1 ←→ SW1 Fa0/1` (trunk, native 99; VLAN 10 allowed)
- `SW2 (3560w/POE) Fa0/2 ←→ SW3 Fa0/2` (trunk, native 99; VLAN 20 allowed)
- `SW2 (3560w/POE) Fa0/3 ←→ Ubuntu VM` (access, VLAN 10)
- `SW2 (3560w/POE) Gi0/1 ←→ SW1 Gi0/1` (trunk, native 99; VLAN 10/20 allowed)
- `SW2 (3560w/POE) Gi0/2 ←→ SW2 Gi0/2` (trunk, native 99; VLAN 10/20 allowed)

After that, everything is now up and running! Lastly, we have to make sure we run all of the show commands and verify that the output is the same on the new 3560w/POE

**Verify**

We’ll run the same commands as we did before the migration on the 2960, but this time on the new 3560w/POE:
...
Everything came back great! Our migration went well, and now we can move on to setting up wireless on my homelab!

`10/11/2025 Update`

## New L3 Access Switch

During the configuration of lab 13 (IP ACLs and Security Services), I attempted to configure my L2 2960 Catalyst switch with the DAI configuration; however, when trying, I was getting an “Invalid input” error, stating that the switch didn’t understand the “ARP” command:

<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/b629b20d-4aa1-47eb-b0bf-e8e518a25798" />
</center>

After checking, it looks like the switch doesn’t support Dynamic ARP Inspection under its current OS:

<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/d7dd36dd-e2f1-4cb9-8e6d-e367cf955841" />
</center>

- With this switch featuring the **LANLITE** image, it can only provide basic Layer 2 security. This is technically an older switch, so it’s a perfect example of real-world upgrades devices need to keep to security standards.

There are a few workarounds for this, but I’ll continue to run into issues when it comes to the last bit of labs, like Quality of Service (QoS), Gateway Redundancy (HSRP), and a few others. The best thing I can do is upgrade the device, and thankfully, I do have my initial L3 Catalyst 3560 core switch (SW1):

<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/e2df07f8-9191-44cf-acb0-bbe5adae747d" />
</center>

- Although it doesn’t have POE, it’ll still offer what I need as an upgraded L3 switch, one of those being the Dynamic ARP Inspection configuration.

After checking the Cisco image, it still has the IPSERVICES file, so we’re in business!
<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/aa1c4219-b073-4aea-a25b-6d65445a12b2" />
</center>

Let’s follow a similar process we did for the last 3560 migration:

### The Migration

With the new device, we need to:

1. **Save Configuration from the 2960**
2. **Stage the New 3560C**
3. **Add the Configuration from 2960 to 3560C**
4. **Physically Swap Devices**
5. **Verify**
...
- These commands will get the general running configuration as well as trunks, VLANs, and anything else.
...
**Add the Configuration from 2960 to 3560w/POE**
With everything good to go, we can configure the new 3560C from my 2960 with this massive configuration paste:
...
!
interface GigabitEthernet0/1
description Fiber Uplink to SW1 Gi0/2
switchport trunk encapsulation dot1q
switchport trunk allowed vlan 10,20,99
switchport mode trunk
ip dhcp snooping trust
...
!
interface GigabitEthernet0/2
description Fiber Uplink to SW2 Gi0/2
switchport trunk encapsulation dot1q
switchport trunk allowed vlan 10,20,99
switchport mode trunk
ip dhcp snooping trust
...
<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/032d5fc2-ef19-441b-8eb2-ff8b4ed77944" />
</center>

<center>
  <img width="400" height="500" alt="new_3560v2" class="center" src="https://github.com/user-attachments/assets/291f88eb-b3f7-46de-a167-e1502731d4bd" />
</center>

Look at this back-to-back view of the switch between devices:

- Swapping them wasn’t bad, I just had to:
    1. Pull out the Ethernet and fiber connections
    2. Remove the 2960 by unscrewing the side screws
    3. Remove the rack mount ears from the 2960
    4. Add the rack mounts to the 3560C
    5. Place the screws on the rack mount for the 3560C
    6. Add the Ethernet and fiber connections

Ultimately, the physical topology didn’t change, only the L3 switch:

- `SW3 (3560C) Fa0/1 ←→ SW1 Fa0/1` (trunk, native 99; VLAN 10 allowed)
- `SW3 (3560C) Fa0/2 ←→ SW3 Fa0/2` (trunk, native 99; VLAN 20 allowed)
- `SW3 (3560C) Fa0/3 ←→ Windows VM` (access, VLAN 20)
- `SW3 (3560C) Gi0/1 ←→ SW1 Gi0/2` (trunk, native 99; VLAN 10/20 allowed)
- `SW3 (3560C) Gi0/2 ←→ SW2 Gi0/2` (trunk, native 99; VLAN 10/20 allowed)

After that, everything is now up and running! Lastly, we have to make sure we run all of the show commands and verify that the output is the same on the new 3560C.

**Verify**

We’ll run the same commands as we did before the migration on the 2960, but this time on the new 3560C:
...
Everything came back solid! The migration went well, and now we can move on to finishing up the last bit of my labs. This should be the last physical topology upgrade; it feels so good to be close to done!
