...
For this lab, we’re going to extend the homelab into the wireless domain. Wireless networking is accomplished with a Wireless Controller (WLC) and an Access Point (AP), such as my Cisco 2504 Wireless LAN Controller (WLC) and Cisco 3602I lightweight AP (AIR-CAP3602I-A-K9). The WLC provides centralized WLAN management, while the AP includes client access.
...
- `WLC Port 1 → SW1 Fa0/5` (trunk, VLAN 99 native, VLAN 100/110/120 allowed for Wi-Fi)
- `AP Ethernet → **SW2 Fa0/4` (trunk, VLAN 99 native, VLAN 100/110/120 allowed for Wi-Fi)
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/95d75049-0bd9-4e0a-bc3f-55f2d396276a" />
</center>

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/5cb004c9-07bd-4ea8-8b1b-1b5403127913" />
</center>
...
- `Virtual Gateway IP Address: 1.1.1.1`: 1.1.1.1 of the WLC IP address, used for client web authentication - isn’t actually used in my homelab.
- `Multicast IP Address: 239.1.1.1`: Used for the WLC to AP communication, the first available multicast IP is `239.1.1.1` - I could update this to a broadcast address, like `255.255.255.255`, but let’s get as close to production as we can.
...
- `Configure a NTP server now? [YES][no]: YES`: We need to say “yes” because the DTLS/cert **time** needs to be synchronized by NTP.
...
corpwifi                         1    100      192.168.100.2   Dynamic No     No
devwifi                          1    110      192.168.110.2   Dynamic No     No
guestwifi                        1    120      192.168.120.2   Dynamic No     No
management                       1    untagged 192.168.1.50    Static  Yes    No
virtual                          N/A  N/A      1.1.1.1         Static  No     No
```
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/62755fe4-dbc2-43fa-ae5c-ae84e05fddb3" />
</center>

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/ca11a30d-4d35-4524-8733-54cf2b83e071" />
</center>
...
| --- | --- | --- | --- | --- |
| CorpWiFi | WPA2-PSK | AES-CCMP | Pre-Shared Key | Internal/Corporate Use |
...
Once the configuration is complete and we’ve verified that WLAN security is working as intended, we should be ready to move on to the next phase of this WLAN endeavor: FlexConnect.

### **FlexConnect on DevWiFi**

Flexconnect was designed for remote branch offices that didn’t have a local WLC. This allowed APs to:

- Stay on a central WLC for control/management (CAPWAP)
- Continue limited service even if the WAN link when t down, but the AP could still connect to clients, and authenticate them.
- Localay switches user traffic to a local VLAN.

However, it’s mostly for legacy environments nowadays, even though it’s still in use today (evolved into “Flex Mode”). For my homelab, we’re going to simulate DevWiFi as a remote site to demonstrate local switching compared to central switching (CorpWiFi/GuestWifi). To do this, let’s:

- **Put the AP in FlexConnect Mode**
- **Enable Local Switching on WLAN 2 (DevWiFi)**
- **Map WLAN 2 to VLAN 110 on the AP (Flex WLAN Mapping)**

This is a very easy set-up, so let’s knock it out:

**Put the AP in FlexConnect Mode**

To start, let’s enable FlexConnect Mode with this simple command:

**WLC**

```bash
(Cisco Controller) > config ap mode flexconnect AP1
```

**Verify**

```bash
(Cisco Controller) > show ap config general AP1
!
! Removed unrelated output
!
AP Mode.......................... FlexConnect
```

Once we’ve verified that FlexConnect is on the AP, we can enable local switching on WLAN 2.

**Enable Local Switching on WLAN 2 (DevWiFi)**

Something we haven’t seen yet, this is how we enable the local-switching of WLAN 2:
...
(Cisco Controller) > config wlan flexconnect central-assoc 2 disable
(Cisco Controller) > config wlan flexconnect local-switching 2 central-dhcp disable
(Cisco Controller) > config wlan enable 2
```

- This configuration will keep the CAPWAP in control, but bridge the client data locally at the AP (on VLAN 110)

From here, we now map the WLAN to the VLAN so the AP can talk to the WLC for FlexConnect.

**Map WLAN 2 to VLAN 110 on the AP (Flex WLAN Mapping)**

Similar to how we mapped the WLANs to the VLANs, we’ll do something very similar here for DevWiFi:

**WLC**

```bash
(Cisco Controller) > config ap flexconnect vlan add 110 acl 0 0 AP1
(Cisco Controller) > config ap flexconnect wlan-vlan enable AP1 2 110
(Cisco Controller) > config ap flexconnect native-vlan AP1 99
```

**Verify**

```bash
show ap config flexconnect AP1
show client detail DevWifi
```

- Here, we want to see that the interface `devwifi` is set to `local Bridging VLAN 110` with FlexConnect Local Switching `enabled`.

If you run into any issues with these commands, you can always configure the mapping in the browser GUI. Once the WLC states that VLAN 110 is now in FlexConnect mode, we’re all set to test and make sure that client devices can connect.
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/0684d0e0-4fa9-425e-97ee-8c185d22aabc" />
</center>

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/765a5418-1ba9-4d30-9061-0d2aebc9d008" />
</center>

**DevWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/f025eb11-2182-47ad-8c40-b02001aa3ffa" />
</center>

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/aa3b2299-e8c9-4176-862a-fe3350267323" />
</center>

**GuestWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/ebfedf07-0b63-438d-afac-58f879a1b087" />
</center>

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/70fe8a7c-e2f7-4423-bcb8-6b3b6d8d1510" />
</center>
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/27744a22-1cff-47b9-bfab-9494549f9dd8" />
</center>

**DevWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/075b0975-7568-4347-be59-bb9eec59f3ab" />
</center>

**GuestWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/344942ef-0aa8-4d59-a1bb-78234a7c28c1" />
</center>
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/57aeeed4-9f6c-4a3f-bc78-397f97ebbc33" />
</center>

**DevWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/d2298a06-34af-40f3-b932-244f616945c4" />
</center>

**GuestWiFi**

<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/4ed328a1-cae4-414b-8880-f9c5913c3e59" />
</center>
...
Once everything is set up/working, any new WLANs shouldn’t have to be configured through the WLC; you can always configure them through the web GUI (which is honestly much easier).
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/e8f351b6-b2b0-49d7-a054-dee27af213ad" />
</center>
...
<center>
  <img width="400" height="500" alt="sw1_default_desktop" class="center" src="https://github.com/user-attachments/assets/09cc6049-911c-4551-a8dc-b290adef8480" />
</center>
